...
PyObject* self_pyarg;
                                      ...
                                      if (PyArg_ParseTuple(args, "O:clear"
                        ,
             &self_pyarg                      ) <= 0)
           return 0;
...
break;
default:
PyErr_Format(PyExc_TypeError,"The 'self' argument of 'clear' can not have pixel type '%s'. Acceptable values are ONEBIT, ONEBIT, ONEBIT, ONEBIT, GREYSCALE, GREY16, RGB, and FLOAT.", get_pixel_type_name(self_pyarg));
return 0;
}
  ...
  DL_EXPORT(void) init_clear(void) {
    Py_InitModule("_clear", _clear_methods);
  }
